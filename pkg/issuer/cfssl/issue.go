...
func (c *CFSSL) buildRequest(crt *v1alpha1.Certificate, template *x509.CertificateRequest, key crypto.PrivateKey) (Request, error) {
	...
	return c.buildAuthenticatedRequest(crt, template, key, spec.AuthKey)
}
...
func (c *CFSSL) buildAuthenticatedRequest(crt *v1alpha1.Certificate,
	...
	secret, err := c.secretsLister.Secrets(c.resourceNamespace).Get(secretSelector.Name)
	if err != nil {
		return nil, err
	}
	...
	if !ok {
		return nil, fmt.Errorf("no data for %q in secret '%s/%s'", secretSelector.Key, secretSelector.Name, c.resourceNamespace)
	}
